/**
 * Sync Veesp client area data into vps-tracker DB
 */
...
import { veespCredentialsString } from './context.js'
...
import {
  ...
  fetchTariffList,
  fetchVpsRecords,
  type VeespBalanceResult,
} from './operations.js'

export interface SyncFromVeespOptions {
  skipTariffs?: boolean
...
export interface SyncFromVeespResult {
  vpsCount: number
  paymentsCount: number
  tariffsCount: number
  newTariffs: { name: string; price: string; providerId: string }[]
  balance: VeespBalanceResult | null
...
export async function syncFromVeesp(
  account: VeespSyncAccount,
  opts: SyncFromVeespOptions = {},
): Promise<SyncFromVeespResult> {
  const { skipTariffs = false, skipVpsPayments = false } = opts
  const { apiBaseUrl, providerId, id: accountId } = account
  const credentials = veespCredentialsString(account)
  if (!apiBaseUrl?.trim() || !account.apiLogin?.trim() || !account.apiPassword) {
  ...
  if (fetchVpsData) {
    for (const record of records) {
      const vps = mapVpsRecordToVps(record, providerId, accountId, fallbackCurrency)
      const id = `vps-veesp-${accountId}-${vps.externalId}`
  ...
  if (fetchVpsData) {
    ...
    for (const item of invoices) {
      const payment = mapInvoiceToPayment(item, accountId, fallbackCurrency)
      if (!payment) continue
      const note = payment.note
      if (existingPayments.has(note)) continue
      const payId = `pay-veesp-${accountId}-${payment.externalId}`
  ...
  if (fetchTariffs) {
    ...
    for (const t of tariffItems) {
      ...
      const tariffId = dcKey
        ? `tariff-veesp-${accountId}-${t.externalId}-${dcKey}`
        : `tariff-veesp-${accountId}-${t.externalId}`
